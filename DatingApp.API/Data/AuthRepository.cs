...
namespace DatingApp.API.Data
{
    public class AuthRepository : IAuthRepository
    {
        ...
        {
            var user = await _context.Users.FirstOrDefaultAsync(x => x.Username == username); // Or default returns null if none found
            if (user == null)
                return null;
        ...
        {
            using (var hmac = new System.Security.Cryptography.HMACSHA512(passwordSalt)) // using will ensure dispose is called when the statement is completed to free resources 
            {
                var computedHash = hmac.ComputeHash(System.Text.Encoding.UTF8.GetBytes(password)); // Hashes the password as an array of bytes
